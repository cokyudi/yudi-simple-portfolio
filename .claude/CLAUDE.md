...
- `hooks/` — shared client hooks (`useChat` for assistant state, `useKeyboardInset` to hold a bottom-anchored `fixed` panel above the mobile keyboard, `useLatestPost` for the language-aware newest-post pick)
...
On mobile Safari the panel is positioned by `hooks/useKeyboardInset.ts` rather than CSS alone: iOS anchors `fixed` elements to the layout viewport while the keyboard shrinks only the visual viewport, so a bottom-pinned panel ends up behind the keyboard. The hook follows `window.visualViewport` and writes `bottom`/`height` straight to the element's style — not through React state, so the scroll events iOS fires all through the keyboard animation don't re-render the message list each frame. The Tailwind `bottom-32` / `h-[28rem]` classes stay as the pre-hydration fallback, and the numbers passed to the hook mirror them. The chat input is `text-base sm:text-sm` for a related reason: iOS zooms the page for any focused input under 16px. Do not "fix" that with `maximum-scale=1` on the viewport meta — it kills pinch-zoom site-wide.
